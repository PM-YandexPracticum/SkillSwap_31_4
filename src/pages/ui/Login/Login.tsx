import { ButtonUI, Input } from '@ui';
import lightBulb from '../../../images/light-bulb.png';
import styles from './Login.module.scss';
import type { LoginUIProps } from './type';

export const LoginUI = ({ handleSubmit }: LoginUIProps) => (
	<div className={styles.page}>
		<h2 className={styles.title}>Вход</h2>
		<div className={styles.content}>
			<form className={styles.form} name='login' onSubmit={handleSubmit}>
				<div className={styles['login-other']}>
					<ButtonUI color='secondary' width='436px'>
						<svg
							width='25'
							height='24'
							viewBox='0 0 25 24'
							fill='none'
							xmlns='http://www.w3.org/2000/svg'>
							<path
								d='M24.5002 12.2658C24.5002 11.2792 24.4185 10.5592 24.2417 9.8125H12.7451V14.2658H19.4934C19.3574 15.3725 18.6227 17.0392 16.99 18.1591L16.9671 18.3082L20.6021 21.0679L20.854 21.0925C23.1668 18.9991 24.5002 15.9191 24.5002 12.2658Z'
								fill='#4285F4'
							/>
							<path
								d='M12.745 23.9997C16.051 23.9997 18.8265 22.933 20.8538 21.093L16.9898 18.1596C15.9558 18.8663 14.568 19.3596 12.745 19.3596C9.50688 19.3596 6.75861 17.2664 5.77892 14.373L5.63532 14.385L1.85558 17.2517L1.80615 17.3863C3.81974 21.3063 7.9558 23.9997 12.745 23.9997Z'
								fill='#34A853'
							/>
							<path
								d='M5.77886 14.3731C5.52036 13.6265 5.37076 12.8264 5.37076 11.9998C5.37076 11.1731 5.52036 10.3731 5.76526 9.62643L5.75841 9.46741L1.93131 6.55469L1.80609 6.61306C0.976196 8.23974 0.5 10.0664 0.5 11.9998C0.5 13.9331 0.976196 15.7597 1.80609 17.3864L5.77886 14.3731Z'
								fill='#FBBC05'
							/>
							<path
								d='M12.745 4.63997C15.0442 4.63997 16.5952 5.6133 17.4796 6.42669L20.9354 3.12C18.813 1.18667 16.051 0 12.745 0C7.9558 0 3.81974 2.69331 1.80615 6.61328L5.76532 9.62665C6.75861 6.73333 9.50688 4.63997 12.745 4.63997Z'
								fill='#EB4335'
							/>
						</svg>
						&nbsp; Продолжить с Google
					</ButtonUI>
					<ButtonUI color='secondary' width='436px'>
						<svg
							width='24'
							height='24'
							viewBox='0 0 24 24'
							fill='none'
							xmlns='http://www.w3.org/2000/svg'>
							<path
								d='M11.6545 5.77578C11.3986 4.3804 12.0583 2.94439 12.8529 1.97664C13.7286 0.908854 15.2314 0.0899883 16.5157 0C16.7326 1.46297 16.1356 2.88836 15.3497 3.8966C14.5066 4.97974 13.0571 5.8197 11.6545 5.77578ZM19.1875 10.832C19.5849 9.72329 20.3722 8.72577 21.5935 8.05273C20.3593 6.5126 18.6265 5.61845 16.9912 5.61845C14.8277 5.61845 13.9129 6.64923 12.4102 6.64923C10.8621 6.64923 9.68774 5.61845 7.81372 5.61845C5.97605 5.61845 4.01993 6.73851 2.77935 8.65142C2.32321 9.3585 2.01422 10.2369 1.8457 11.2153C1.3781 13.96 2.07659 17.535 4.16018 20.7094C5.17325 22.2497 6.52359 23.9847 8.28766 23.9998C9.85912 24.0152 10.3049 22.9951 12.4323 22.9846C14.5628 22.9725 14.9666 24.0104 16.5359 23.9954C18.3005 23.9805 19.7252 22.0604 20.7383 20.5203C21.4596 19.4151 21.7324 18.8568 22.2933 17.6073C19.4412 16.5317 18.2681 13.3889 19.1875 10.832Z'
								fill='#253017'
							/>
						</svg>
						&nbsp; Продолжить с Apple
					</ButtonUI>
				</div>
				<div className={styles.divider}>
					<span className={styles.line} />
					или
					<span className={styles.line} />
				</div>
				<div className={styles.login}>
					<div className={styles.inputs}>
						<Input
							label='Email'
							placeholder='Введите email'
							variant='default'
							className={styles.input}
						/>
						<Input
							label='Пароль'
							placeholder='Введите ваш пароль'
							variant='password'
							type='password'
							className={styles.input}
						/>
					</div>
					<ButtonUI color='primary' width='436px' htmlType='submit'>
						Войти
					</ButtonUI>
					<a href='#1' className={styles['signup-link']}>
						Зарегистрироваться
					</a>
				</div>
			</form>
			<div className={styles.onboarding}>
				<img
					src={lightBulb}
					alt='Light Icon'
					style={{ marginBottom: '40px' }}
				/>
				<h2 className={styles.title} style={{ marginBottom: '12px' }}>
					С возвращением в SkillSwap!
				</h2>
				<p className={styles.subtitle}>
					Обменивайтесь знаниями и навыками с другими людьми
				</p>
			</div>
		</div>
	</div>
);
